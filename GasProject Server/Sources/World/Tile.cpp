...
bool Tile::RemoveObject(Object *obj) {
	if (removeObject(obj)) {
		...
		diff->objId = obj->ID();
		AddDiff(diff, obj->GetInvisibility());
		return true;
	}
	return false;
}
...
bool Tile::MoveTo(Object *obj) {
	...
	lastTile->AddDiff(std::move(relocateAwayDiff), obj->GetInvisibility());
	...
	AddDiff(moveDiff, obj->GetInvisibility());
...
void Tile::PlaceTo(Object *obj) {
	...
	if (lastTile) {
		...
		relocateAwayDiff->objId = obj->ID();
		relocateAwayDiff->newCoords = pos;
		lastTile->AddDiff(relocateAwayDiff, obj->GetInvisibility());
	}
	addObject(obj);
	...
	AddDiff(relocateDiff, obj->GetInvisibility());
}
...
void Tile::AddDiff(std::shared_ptr<network::protocol::Diff> diff, uint invisibility) {
	EXPECT(uf::CreateSerializableById(diff->Id())); // debug
	differencesWithInvisibility.push_back({diff, invisibility});
}

void Tile::ClearDiffs() {
	differencesWithInvisibility.clear();
}
