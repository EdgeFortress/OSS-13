...
public:
	...
	void AddDiff(std::shared_ptr<network::protocol::Diff> diff, uint invisibility);
    const std::vector<std::pair<std::shared_ptr<network::protocol::Diff>, uint>> &GetDifferencesWithInvisibility() const { return differencesWithInvisibility; }
    void ClearDiffs();
...
private:
    ...
    std::vector<std::pair<std::shared_ptr<network::protocol::Diff>, uint>> differencesWithInvisibility;
